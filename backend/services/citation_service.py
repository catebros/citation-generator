...
from services.validators import (CitationTypeValidator,
                                 ParameterValidator)
...
class CitationService:
    ...
    def create_citation(self, project_id: int, data: dict) -> Citation:
        """Create new citation with validation and duplicate detection."""
        # Validate required parameters
        if project_id is None:
            raise HTTPException(
                status_code=400, detail="project_id is required for citation creation"
            )
        ...
        if data is None:
            raise HTTPException(
                status_code=400, detail="data is required for citation creation"
            )

        # Validate incoming citation data using Pydantic
        try:
            validated_data = CitationCreate(**data)
        except ValidationError as e:
            raise HTTPException(status_code=400, detail=str(e))

        # Check if identical citation exists in project
        duplicate = self._citation_repo.find_duplicate_citation_in_project(
            project_id, data
        )
        if duplicate:
            raise HTTPException(
    ...
    def update_citation(
        self, citation_id: int, project_id: int, data: dict
    ) -> Citation:
        """Update citation with validation and duplicate detection."""
        ...
        duplicate = self._citation_repo.find_duplicate_citation_in_project(
            project_id, merged_data
        )
        if duplicate and duplicate.id != citation_id:
            raise HTTPException(
                status_code=409,
                detail="An identical citation already exists in this project",
            )
        ...
        type_change = (
            new_type is not None and new_type.lower() != current_type.lower()
        )
        ...
        if type_change:
            CitationTypeValidator.validate_type_change(
                citation_dict, new_type, current_type
            )

        # Perform the update
        updated = self._citation_repo.update(
            citation_id=citation_id, project_id=project_id, **citation_dict
        )
        if not updated:
    ...
    def delete_citation(self, citation_id: int, project_id: int) -> Dict[str, str]:
        """Delete citation and remove project associations."""
        ParameterValidator.validate_required(citation_id, "citation_id")
        ParameterValidator.validate_required(project_id, "project_id")
        ...
        success = self._citation_repo.delete(
            citation_id=citation_id, project_id=project_id
        )
        if not success:
    ...
    def format_citation(self, citation: Citation, format_type: str = "apa") -> str:
        ...
        formatters = {"apa": APAFormatter, "mla": MLAFormatter}
