...
def test_stress_create_100_citations():
    ...
    for i in range(100):
        ...
        base_data = {
            "type": citation_type,
            "authors": [
                f"Author Smith {chr(65 + i % 26)}",
                f"Co-Author Jones {chr(65 + (i+1) % 26)}",
            ],
        ...
        if response.status_code != 201:
            print(
                f"Error in citation {i+1} (type: {citation_type}): {response.status_code}"
            )
        ...
        ), f"Error in citation {i+1}: {response.status_code}"
    ...
    print("\nSTRESS TEST RESULTS")
    print("=" * 50)
    print(f"Total citations created: {len(citation_ids)}")
...
def test_stress_concurrent_bibliography():
    ...
    for format_type in formats:
        ...
        bibliography_response = client.get(
            f"/projects/{project_id}/bibliography?format={format_type}"
        )
        ...
        if bibliography_response.status_code != 200:
            print(
                f"Error in {format_type} bibliography: {bibliography_response.status_code}"
            )
            try:
        ...
        print(
            f"{format_type.upper()} bibliography generated in {generation_time:.3f} seconds"
        )
        ...
        if generation_time < 3.0:
            print(
                f"Performance GOOD for {format_type.upper()}: {generation_time:.3f}s < 3.0s"
            )
        else:
            print(
                f"Performance SLOW for {format_type.upper()}: {generation_time:.3f}s >= 3.0s"
            )

        assert (
            generation_time < 5.0
        ), f"Bibliography generation {format_type} too slow: {generation_time:.3f}s"
