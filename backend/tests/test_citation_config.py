...
def test_get_required_for_citation_types_each_has_fields():
    ...
    for citation_type, fields in all_requirements.items():
        ...
        assert len(fields) > 0, f"{citation_type} should have at least one required field"
...
def test_all_required_fields_are_strings():
    ...
    for citation_type, fields in all_requirements.items():
        for field in fields:
            assert isinstance(field, str), f"Field '{field}' in {citation_type} should be a string"
...
def test_no_duplicate_fields_in_any_type():
    ...
    for citation_type, fields in all_requirements.items():
        ...
        assert len(unique_fields) == len(fields), f"{citation_type} should not have duplicate fields"
...
def test_common_fields_across_all_types():
    ...
    for citation_type, fields in all_requirements.items():
        ...
        assert common_fields.issubset(fields_set), \
