...
def test_create_project_and_citations_workflow():
    ...
    try:
        project_response = client.post(
            "/projects", json={"name": "Integration Test Project"}
        )
...
def test_bibliography_format_change_apa_to_mla():
    ...
    try:
        ...
        mock_proj_service.generate_bibliography_by_project.return_value = (
            apa_bibliography
        )
...
def test_full_workflow_project_citations_bibliography():
    ...
    try:
        project_response = client.post(
            "/projects", json={"name": "Full Workflow Project"}
        )
        assert project_response.status_code == 201
        project_id = project_response.json()["id"]
        ...
        bib_url = f"/projects/{project_id}/bibliography?format_type=apa"
        bibliography_response = client.get(bib_url)
...
def test_update_citation_and_regenerate_bibliography():
    ...
    try:
        ...
        updated_bibliography = {
            ...
            "format_type": "apa",
            "bibliography": [
                "Smith, J. (2024). Updated Research Methods. New Publisher."
            ],
            "citation_count": 1,
        }
        mock_proj_service.generate_bibliography_by_project.return_value = (
        ...
        bib_url = "/projects/1/bibliography?format_type=apa"
        bibliography_response = client.get(bib_url)
        assert bibliography_response.status_code == 200
